...
class WebPackGenerator {
    ...
    configureVisualPlugin(tsconfig, visualPackage) {
        ...
        this.webpackConfig.output.chunkFilename = "[name].js";
        if (tsconfig.compilerOptions.out) {
            ...
            this.webpackConfig.plugins.push(
                new webpack.WatchIgnorePlugin([
                    visualPluginPath,
                    /globalize-compiled-data/g,
                    /\.tmp-globalize-webpack/g
                ])
            );
        }
    }
    ...
    async appendPlugins(options, visualPackage, tsconfig) {
        ...
        const folders = await fs.readdir(resourcesDir);
        ...
        this.webpackConfig.plugins.push(
            ...
            new globalizePlugin({
                ...
                supportedLocales: folders.map(folder => folder.split("-")[0]).filter(locale => locale === "ru" || locale === "en"), // locales that should be built support for.
                // cldr: function() {}, // CLDR data (optional)
                // messages: path.join(resourcesDir, "[locale]", "resources.resjson"), // messages (optional)
                // timeZoneData: function() {}, // time zone data (optional)
                output: "globalize-compiled-data-[locale].js", // build output.
                // moduleFilter: filterFunction, // filter for modules to exclude from processing
                tmpdirBase: config.build.dropFolder, // optional for non create-react-apps
            }),
            new webpack.WatchIgnorePlugin([
                /globalize-compiled-data/g,
                /\.tmp-globalize-webpack/g
